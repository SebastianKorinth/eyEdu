...
EyEduImportWordAoiLabels <- function(extra.aoi = NULL,
  ...
  if (is.null(eyEdu.data$aoi.info)){
    return("There are no AoIs defined yet! Please run EyEduDefineWordAois() or import existing aoi files!")
  } 
  
  for(aoi.counter in 1: length(eyEdu.data$aoi.info)){
    if(sparse.aoi.definition == TRUE){
    # extracts the stimulus id variable (i.e., the third element of the image name)
    stim.id.var <- unlist(strsplit(eyEdu.data$aoi.info[[aoi.counter]]$image.name[1], "[_]"))[[3]]
    # get the actual stimulus from the trial.info data frame of the first participant
    stim.index <- which(eyEdu.data$participants[[1]]$trial.info$stimulus.id == stim.id.var)
    stimulus <- eyEdu.data$participants[[1]]$trial.info$stimulus.message[stim.index]
    } else {
      stim.id.var <- unlist(strsplit(eyEdu.data$aoi.info[[aoi.counter]]$image.name[1], "[_]"))[[3]]
